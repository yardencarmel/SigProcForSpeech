...
def Q1():
    ...
    # Part C
    # STFT magnitude (50 ms windows, 3 ms hop) displayed in dB as requested.
    plt.figure(figsize=(12, 3))
    win_length = int(0.050 * sr)
    hop_length = int(0.003 * sr)
    n_fft = 1 << (win_length - 1).bit_length()   # smallest power-of-two >= win_length
    ...
    plt.xlabel("time [s]")
    plt.ylabel("MFCC index (C0...C40)")
    plt.yticks(np.arange(40), [f"C{i}" for i in range(40)])
    plt.title("MFCCs (40 coefficients)")
...
def Q2():
    ...
    print(
        "Train gender counts:",
        {g: list(np.array([gender_map[s] for s in spk_train])).count(g) for g in ("M", "F")},
    )
    print(
        "Test  gender counts:",
...
# --------- feature extractors ----------
def mfcc_mean(y, sr, n_mfcc=40, win_ms=50, hop_ms=3):
...
def build_dataset_for_speakers(speaker_ids, gender_map, kind, max_utts_per_speaker=20):
    X, y, meta = [], [], []
    rng = np.random.default_rng(RNG_SEED)  # deterministic sampling when truncating to max_utts_per_speaker
