...
sentences.  The qualitative trend matches the base article's Fig.\,3
(right panel): WER decreases monotonically as $s$ becomes more
negative, and the benefit is largest in the low-NFE regime-at
NFE = 8, $s=-1.0$ yields a 67\,\% WER reduction over $s=+0.4$.
Conversely, SIM-o rises as $s$ moves toward positive values
(0.784 at $s{=}{-}0.8$ vs.\ 0.836 at $s{=}0.0$).  This reveals an
intelligibility-fidelity trade-off inherent to sway sampling:
negative $s$ concentrates ODE evaluations near the noisy end
($t\approx 0$), where coarse structure such as word identity is
resolved, but leaves fewer steps near the clean end ($t\approx 1$),
where fine-grained speaker characteristics are refined.  Positive $s$
reverses this allocation, improving speaker similarity at the cost of
intelligibility.
...
Our $s=-1$, $\text{NFE} = 32$ result (WER\,=\,7.4\,\%, SIM-o\,=\,0.802) is the
...
The optimal $s$ shifts from $-1$ to $-0.8$ in our setup, likely a
small-sample artefact.  For all Extension~2 experiments we retain
...
$s=-1.0$, NFE = $32$.  WER improves monotonically with stronger
guidance (17.2\,\%$\to$3.7\,\%), while SIM-o peaks at
$\lambda=1.0$-$1.5$ and then falls (0.835$\to$0.774), revealing
a clear intelligibility-similarity trade-off.  The default
$\lambda=2.0$ (WER\,=\,7.4\,\%, SIM-o\,=\,0.802) sits at the elbow
of this trade-off, providing empirical justification for the base
article's choice.
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{\linewidth}
    \centering
    \includegraphics[width=\linewidth]{results/noise_inject/method_A/tradeoff_wer_simb_a.png}
  ...
  \begin{subfigure}[b]{\linewidth}
    \centering
    \includegraphics[width=\linewidth]{results/noise_inject/method_A/tradeoff_wer_simb_b.png}
