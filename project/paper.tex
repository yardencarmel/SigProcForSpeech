...
\textbf{Extension\,2} adapts the SDEdit framework~\cite{song2022sdedit} to
flow-matching TTS by biasing the ODE starting point toward a style reference
while conditioning on an identity reference; four variants (Methods A--D)
spanning initialisation and vector-field manipulation are evaluated.
...
\begin{enumerate}[leftmargin=*,label=(\arabic*)]
  ...
  \item \textbf{Noise-biased style transfer} (Extension\,2): four methods
    that manipulate either the ODE starting point or the vector field to
    inject style from a second reference, without touching the identity
    conditioning.
...
\begin{equation}
  \frac{dx}{dt} = v_\theta(x_t,\,c,\,t), \quad
  x_0 \sim \mathcal{N}(0,I), \quad t \in [0,1],
  \label{eq:ode}
\end{equation}
where $v_\theta$ is a DiT vector field and $c$ is the reference mel
spectrogram concatenated to the noisy input along the time axis.
...
\begin{equation}
  v_{\rm guided} = v_\theta(x,\varnothing,t)
    + \lambda\bigl[v_\theta(x,c,t)-v_\theta(x,\varnothing,t)\bigr],
  \label{eq:cfg}
\end{equation}
...
\noindent\includegraphics[width=\linewidth]{results/plots/sway_pdf.png}\par
{\captionsetup{width=\linewidth}%
\captionof{figure}{%
  ODE timestep probability densities for each sway coefficient.
  Negative $s$ concentrates evaluations near $t=0$ (the noisy end);
  the shape matches Fig.\,2 of the base article.}}
\label{fig:sway_pdf}


\noindent\includegraphics[width=\linewidth]{results/plots/sway_sampling.png}\par
{\captionsetup{width=\linewidth}%
\captionof{figure}{%
  WER and SIM-o vs.\ sway coefficient for NFE$\in\{8,16,32\}$.
...
\begin{equation}
  \mathbf{m}{(w)} = (1-w)\,\mathbf{m}_A + w\,\mathbf{m}_B,
  \quad w\in[0,1].
  \label{eq:blend}
\end{equation}
...
clipped, ~1\,s garbled outputs (100\,\% WER at $w\in(0.1,0.9)$ in our v1
...
\listenbox{results/emotion\_transfer/audio/w0.00.wav}{%
  Baseline ($w=0$): does the output sound like reference A
...
\listenbox{results/emotion\_transfer/audio/w1.00.wav}{%
  Full style ($w=1$): does the output sound like reference B
  (Chinese-accented English)? Rate how different it sounds from $w=0$.}

\noindent\includegraphics[width=\linewidth]{results/plots/emotion_weight.png}\par
{\captionsetup{width=\linewidth}%
\captionof{figure}{SIM-o and MCD as a function of blend weight $w$.
...
(output moves away from $A$) as $w$ grows.  Duration also shifts from
$A$'s pace (12.3\,s) toward $B$'s (13.3\,s) at $w\geq0.75$, reflecting
B's conditioning mel's influence on prosodic rhythm.

The $w=0.5$ artefact (WER\,=\,15\,\%) is noteworthy: reference A's
...
\noindent\includegraphics[width=\linewidth]{results/noise_inject/method_A/tradeoff_wer_simb.png}\par
{\captionsetup{width=\linewidth}%
\captionof{figure}{%
  Method~A: WER vs.\ SIM-B scatter for all $(\alpha, s)$ pairs in the
...
\midrule
Ext.\,1 ($w=0.25$) & 0\,\% & \multicolumn{2}{c}{SIM-o\,=\,0.896} & 707 \\
\bottomrule
\end{tabular}
\end{center}


\noindent\includegraphics[width=\linewidth]{results/noise_inject/comparison/comparison_scatter.png}\par
{\captionsetup{width=\linewidth}%
\captionof{figure}{%
  WER vs.\ SIM-B for all inferences across Methods A--D.
...
complementary: it achieves the lowest MCD (707 at $w=0.25$) and the
...
% ── Full-width figures (outside multicols for full page width) ──────────────
\begin{figure}[h]
  \centering
  \begin{subfigure}[t]{0.48\textwidth}
    \includegraphics[width=\textwidth]{results/plots/cfg_strength.png}
  ...
  \hfill
  \begin{subfigure}[t]{0.48\textwidth}
    \includegraphics[width=\textwidth]{results/noise_inject/noise_inject_sweep.png}
    ...
    \label{fig:sweep}
  \end{subfigure}
  \caption{Left: classifier-free guidance ablation.  Right: Method~A
    ($\alpha$--sway) sweep.  Negative sway extends the intelligible
    $\alpha$ range by providing more ODE steps near the noisy initialisation.}
  \label{fig:wide}
\end{figure}
