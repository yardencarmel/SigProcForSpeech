...
\author{
	Y.\ Carmel\textsuperscript{1} \quad N. Dahan\textsuperscript{1}\\[6pt]
	\textsuperscript{1}\textit{The Iby and Aladar Fleischman Faculty of Engineering,}\\
	\textit{Tel Aviv University, Tel Aviv 69978, Israel}\\[4pt]
	\small\texttt{yardencarmel@mail.tau.ac.il}\\
	\small\texttt{nitzandahan1@mail.tau.ac.il}\\[6pt]
	\small Code: \url{https://github.com/yardencarmel/SigProcForSpeech}
}
...
classifier-free guidance ablations, confirming that negative sway ($s \leq -0.4$) with NFE$=32$
achieves near-zero word error rate.  We then introduce two complementary
...
from a short reference clip without speaker-specific training - is a central
challenge in modern text-to-speech (TTS).
Early neural TTS systems such as Tacotron~2~\cite{shen2018tacotron2} and FastSpeech~2~\cite{ren2021fastspeech2} required
speaker-specific fine-tuning and could not generalise to unseen voices.
Autoregressive (AR) models like VALL-E~\cite{wang2023neural} addressed this
by predicting discrete speech tokens, but they suffer from high inference
latency and exposure bias.
Non-autoregressive (NAR) models such as Voicebox~\cite{le2024voicebox} and
NaturalSpeech~3~\cite{ju2024naturalspeech3} improved speed through parallel
generation, yet they relied on complex pipelines: phoneme aligners, duration
predictors, or factorised codec representations.

F5-TTS~\cite{chen2025f5tts} simplifies this pipeline significantly.
It is a fully non-autoregressive system based on Conditional Flow
Matching~\cite{lipman2023flow} and a Diffusion Transformer (DiT).
Instead of requiring a phoneme aligner or duration predictor, it pads the
raw character sequence with filler tokens to match the mel length and lets
the DiT learn alignment implicitly through a speech-infilling objective.
A ConvNeXt~V2 text encoder refines the character embeddings before they
enter the transformer, and an inference-time Sway Sampling strategy biases
the ODE solver toward early timesteps, boosting intelligibility without
retraining.  Together, these choices yield near-perfect transcription
accuracy with only 32 neural function evaluations.
...
\begin{align}
  ...
  \label{eq:ode}
\end{align}
The ODE transforms a random noise sample
$x_0\sim\mathcal{N}(0,I)$ into a clean mel spectrogram $x_1$.
At every timestep $t$, the learned vector field $v_t$ tells the solver
which direction to move, given the current state and a conditioning signal
$c=(x_{\mathrm{ref}},\,z_{\mathrm{ref{\cdot}gen}})$ - the reference
speaker's mel concatenated with the target character sequence along the
time axis.
The model is trained with the Conditional Flow Matching
objective~\cite{lipman2023flow}: rather than learning to reverse a
many-step noising process as in DDPM~\cite{ho2020ddpm}, it learns the
velocity $v_t$ along a straight path that connects each noise sample to
its corresponding clean mel, making both training and inference simpler.

At inference time, classifier-free guidance (CFG)~\cite{ho2022cfg}
strengthens the influence of the conditioning signal.  The model is
evaluated twice per ODE step: once with the full conditioning $c$
(reference mel + text) and once with the conditioning dropped
($\varnothing$).  The difference between these two outputs isolates the
effect of the conditioning; scaling it by a factor $\lambda$ controls how
strongly the output follows the reference speaker:
\begin{align}
  v_{\rm guided} &= v_t(x,\varnothing)\notag\\
    &\quad+ \lambda\bigl[v_t(x,c)-v_t(x,\varnothing)\bigr].
  \label{eq:cfg}
\end{align}
When $\lambda=1$ the conditioning acts at its natural strength; $\lambda>1$
amplifies it, improving speaker fidelity at the risk of reduced
intelligibility (as confirmed by the ablation in Section~\ref{sec:baseline}).
...
measures intelligibility.  Because WER is computed by a neural ASR
model rather than human transcription, it may over- or under-count
errors -- particularly on short evaluation sets where a single
misrecognition shifts the average substantially.
...
\begin{table}[htbp]
\centering
\caption{Sway sampling ablation: mean WER (\%) and SIM-o over
...
$+0.4$ & 32 & 67.8 & 0.623  \\
$0.0$  & 32 & 26.1 & 0.735  \\
$-0.4$ & 32 & \textbf{3.7} & 0.848  \\
$-0.8$ & 32 & \textbf{3.7} & 0.848  \\
$-1.0$ & 8  & \textbf{9.8} & 0.842  \\
$-1.0$ & 32 & \textbf{3.7} & 0.835  \\
\bottomrule
\end{tabular}
\end{table}

Key finding: at NFE\,=\,32, all negative sway coefficients
($s\leq-0.4$) achieve 3.7\,\% mean WER -- the residual error is
driven entirely by a single persistent Whisper transcription error on
one of the three test sentences.  The qualitative trend matches the
base article's Fig.\,3 (right panel): WER decreases as $s$ becomes
more negative, and the benefit is most pronounced in the low-NFE
regime -- at NFE\,=\,8, $s{=}{-}1.0$ reduces WER from 100\,\% ($s{=}{+}0.4$)
to 9.8\,\%.  Among intelligible configurations ($s\leq-0.4$, NFE\,=\,32),
SIM-o is consistently high (0.835--0.848) with only a slight decrease
at $s{=}{-}1.0$.  Positive sway severely degrades both intelligibility
and speaker similarity even at NFE\,=\,32 (WER\,=\,67.8\,\%,
SIM-o\,=\,0.623 at $s{=}{+}0.4$), confirming that concentrating ODE
evaluations near the noisy end ($t\approx 0$) is critical for
resolving coarse speech structure.
Note that Fig.\,3 of the base article reports results for 155\,M parameters small
models on Seed-TTS \emph{test-zh} (Mandarin), whereas our sweep uses
the 336\,M base model on three English sentences; the trend's
consistency across model scale and language strengthens the finding.
Our $s=-1$, $\text{NFE} = 32$ result (WER\,=\,3.7\,\%, SIM-o\,=\,0.835) is the
closest point of comparison with the base article's Table\,2, which
reports WER\,=\,1.83\,\% and SIM-o\,=\,0.67 on Seed-TTS \emph{test-en}
under identical hyperparameters; the gap is attributable to our much
smaller evaluation set (3 vs.\ 1\,088 utterances) and different speaker
embedding model (WavLM-Base-Plus vs.\ WavLM-large-based).
For all Extension~2 experiments we retain
the F5-TTS default $s=-1.0$ to ensure comparability with the base
article.  Figure~\ref{fig:sway_pdf} illustrates the resulting
timestep densities.
...
The base article adopts a default CFG strength of $\lambda=2.0$ but
does not sweep this hyperparameter.  Table~\ref{tab:cfg} fills that
gap by evaluating $\lambda \in \{1.0,1.5,2.0,2.5,3.0\}$ at fixed
$s=-1.0$, NFE = $32$.  All CFG strengths achieve 0\,\% WER,
confirming robust intelligibility across the tested range.
SIM-o decreases modestly from 0.851 at $\lambda=1.0$ to 0.826 at
$\lambda=3.0$, indicating that stronger guidance slightly reduces
speaker fidelity.  The default $\lambda=2.0$ (SIM-o\,=\,0.835)
sits at a reasonable operating point, balancing fidelity with the
conditioning-amplification benefit described in Section~\ref{sec:cfm}.

\begin{table}[htbp]
\centering
\caption{CFG strength ablation: mean WER (\%) and SIM-o.}
\label{tab:cfg}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}crr@{}}
\toprule
...
\begin{figure}[htbp]
  ...
  \caption{CFG strength ablation: WER remains at 0\,\% across all
    $\lambda$ values; SIM-o decreases modestly with stronger guidance,
    confirming $\lambda=2.0$ as a good operating point.}
  \label{fig:cfg}
\end{figure}
...
F5-TTS conditions the DiT on a reference mel by concatenating it
(feature-axis) with the noisy input and projecting the result.  Inspection of \texttt{cfm.py} (lines 106-109) reveals
a dormant branch:
\begin{equation*}
...
\begin{equation}
  ...
  \label{eq:blend}
\end{equation}
Both mels are extracted using the same filterbank configuration
that the model was trained on (24\,kHz, 100 channels, hop\,=\,256,
Slaney norm).  This means the blended tensor has the same scale and
frequency layout the transformer expects, so it remains
in-distribution - even though the model never saw a blended
conditioning signal during training.

\textbf{Why not blend waveforms or invert first?}
We considered two simpler alternatives but both fail.
Mixing raw waveforms causes phase cancellation between the two signals,
destroying the spectral structure the model relies on.
Blending in mel space and then converting back to audio via the Vocos
vocoder also fails: the vocoder has never seen a blended mel during
training, so at intermediate weights ($\alpha\in(0.1,0.9)$) it produces
short, garbled clips (100\,\% WER in our early experiments).
Our solution - injecting the blended mel directly into the transformer's
conditioning input - avoids the vocoder entirely and keeps the signal
in the representation space the model already understands.
...
\begin{table}[htbp]
\centering
\caption{Direct mel injection results.  SIM-o is cosine similarity
...
At $\alpha=0.5$ the output is fully intelligible (WER\,=\,0\,\%),
though brief blending noise is audible at the utterance onset --
the two conflicting conditioning mels create a transient artefact
that does not affect the transcribed word sequence.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{results/extension_2/method_A/method_A_sweep.png}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{\linewidth}
    \centering
    \includegraphics[width=\linewidth]{results/extension_2/method_A/tradeoff_wer_simb_a.png}
    \caption{Low-WER region (WER $\leq$ 200\%).  The Pareto-optimal region
    (low WER, high SIM-B) is at small positive $\alpha$ with negative sway.}
    \label{fig:tradeoff-a}
  \end{subfigure}
  \par\medskip
  \begin{subfigure}[b]{\linewidth}
    \centering
    \includegraphics[width=\linewidth]{results/extension_2/method_A/tradeoff_wer_simb_b.png}
    \caption{High-WER region (WER $\geq$ 500\%).  These points correspond to
    large $\alpha$ values where $B$'s temporal structure dominates the
    initialisation, producing unintelligible speech.}
    \label{fig:tradeoff-b}
  \end{subfigure}
  \caption{Method~A: WER vs.\ SIM-B scatter for all $(\alpha, s)$ pairs in the
  fine-grained sweep.  Each point is one inference; colour encodes $\alpha$. WER above 100\% symbolizes words generation that are not present at the target's text input.}
  \label{fig:tradeoff}
\end{figure}
...
\textbf{Formulation.}
At each ODE step the transformer is evaluated twice - once
conditioned on the identity reference $\mathbf{m}_A$ and once on the
style reference $\mathbf{m}_B$ - to produce two velocity proposals
$v_A$ and $v_B$.  These are blended into a single update direction:
\begin{equation}
  v_{\rm style} = v_A + g\,(v_B - v_A),
  \label{eq:methb}
\end{equation}
where $v_A = v_{\rm guided}(x,\mathbf{m}_A,t)$,\;
$v_B = v_{\rm guided}(x,\mathbf{m}_B,t)$, and $g$ is the style
guidance scale.  The resulting $v_{\rm style}$ replaces the usual
velocity $v_t$ in the ODE solver (Eq.~\ref{eq:ode}): at every
integration step the solver advances $x$ in the direction of
$v_{\rm style}$ rather than the single-speaker velocity, steering the
trajectory toward $B$'s acoustic characteristics while remaining
anchored to $A$'s identity.
This is structurally analogous to classifier-free guidance
(Eq.~\ref{eq:cfg}), but the roles differ: $\lambda$ in CFG scales
the effect of conditioning vs.\ no conditioning, whereas $g$ here
scales the shift from one speaker's conditioning toward another's.
We use a distinct symbol to reflect this difference.
Because computing $v_{\rm style}$ requires two transformer forward
passes per step (one for each conditioning mel), the computational
cost doubles relative to standard inference.
...
\textbf{Method C (Scheduled Conditioning)} modifies the conditioning mel
that is fed to the transformer's vector field $v_t$
(Eq.~\ref{eq:ode}) at each ODE step, switching it from the style
reference to the identity reference at a threshold timestep $t^*$:
\begin{equation}
  c(t) = \begin{cases} \mathbf{m}_B & t < t^* \\ \mathbf{m}_A & t \geq t^* \end{cases}
  \label{eq:methc}
\end{equation}
The hope is that conditioning on $\mathbf{m}_B$ during early steps
imprints $B$'s stylistic characteristics, while switching to
$\mathbf{m}_A$ for the remaining steps restores $A$'s identity.
However, as discussed in Section~\ref{sec:baseline}, early ODE steps
($t\approx 0$) primarily resolve coarse structure such as word
identity, not fine-grained speaker traits - so conditioning on
$\mathbf{m}_B$ early actually imprints $B$'s identity wholesale,
leaving too little integration time for $\mathbf{m}_A$ to recover.
...
overwhelms the Euler integrator; a smooth cosine schedule may mitigate this, but is left as future work.
...
\begin{table}[htbp]
\centering
\caption{Best WER\,=\,0\,\% result per method at $s=-1$.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{results/extension_2/comparison/comparison_scatter.png}
  \caption{Identity preservation (SIM-A) vs.\ style acquisition (SIM-B)
  for all inferences across Methods~A-D.  Each point is one inference;
  the ideal operating region is the upper-right corner (high SIM-A and
  high SIM-B).  Method~B achieves the highest SIM-B at WER\,=\,0\,\%;
  Methods~C and~D achieve high SIM-B only at the cost of unintelligible
  output.}
  \label{fig:scatter}
\end{figure}
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Baseline Reproduction} (Section~\ref{sec:baseline}): intelligible but
  ...
  \item \textbf{Ext.\,1, $\alpha{=}0.5$}: a moderate acoustic morph is
    ...
    Brief blending noise is audible at the utterance onset;
    a faint buzzing artefact persists in the background.
...
and compute-intensive retraining of the 335\,M-parameter model. All in all,
based on preliminary tests run in the scope of this work, using Hebrew text 
either as input or zero shot yields a gibberish voice (no-word spoken) of the target.
...
and might follow a smoothly changing conditioning field provided it is not
...
Section~\ref{sec:security} deserves some empirical study; gradient-based
...
Y.\ Chen, Z.\ Niu, Z.\ Ma, K.\ Deng, C.\ Wang, J.\ Zhao,
K.\ Yu, and X.\ Chen,
...
C.\ Wang, S.\ Chen, Y.\ Wu, Z.\ Zhang, L.\ Zhou, S.\ Liu,
Z.\ Chen, Y.\ Liu, H.\ Wang, J.\ Li, L.\ He, S.\ Zhao, and F.\ Wei,
``Neural Codec Language Models are Zero-Shot Text to Speech Synthesizers,''
arXiv:2301.02111, 2023.

\bibitem{le2024voicebox}
M.\ Le, A.\ Vyas, B.\ Shi, B.\ Karrer, L.\ Sari, R.\ Morber,
M.\ Rivi\`ere, M.\ Elber, A.\ Sagar, W.-N.\ Hsu, et al.,
``Voicebox: Text-Guided Multilingual Universal Speech Generation at Scale,''
in \textit{Proc.\ NeurIPS 2024}, 2024.
arXiv:2306.15687.

\bibitem{ju2024naturalspeech3}
Z.\ Ju, Y.\ Wang, K.\ Shen, X.\ Tan, D.\ Xin, D.\ Yang,
Y.\ Liu, Y.\ Leng, K.\ Song, S.\ Tang, et al.,
``NaturalSpeech 3: Zero-Shot Speech Synthesis with a Factorized
Codec and Diffusion Models,''
in \textit{Proc.\ ICML 2024}, 2024.
arXiv:2403.03100.

\bibitem{shen2018tacotron2}
J.\ Shen, R.\ Pang, R.\ J.\ Weiss, M.\ Schuster, N.\ Jaitly,
Z.\ Yang, Z.\ Chen, Y.\ Zhang, Y.\ Wang, R.\ Skerrv-Ryan, et al.,
``Natural TTS Synthesis by Conditioning WaveNet on Mel Spectrogram
Predictions,''
in \textit{Proc.\ ICASSP 2018}, 2018.
arXiv:1712.05884.

\bibitem{ren2021fastspeech2}
Y.\ Ren, C.\ Hu, X.\ Tan, T.\ Qin, S.\ Zhao, Z.\ Zhao,
and T.-Y.\ Liu,
