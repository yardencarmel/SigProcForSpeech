...
Output directory: results/noise_inject/method_A/
...
def infer_noise_inject(
    ...
    with torch.inference_mode():
        generated, _ = model.sample(
            ...
            seed=seed,
            noise_source=mel_B_3d if noise_level > 0.0 else None,
...
def compute_wer(wav_path, expected_text, device):
    try:
...
def compute_sims(gen_wav, ref_A, ref_B, device):
    try:
...
def run_sweep(args):
    device  = args.device
    out_dir = PROJECT_ROOT / "results" / "noise_inject" / "method_A"
    ...
    for i, row in enumerate(rows):
        ...
        if isinstance(row["wer"], (int, float)):
