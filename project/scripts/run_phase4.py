...
def compute_wer(wav_path: str, expected_text: str, device: str) -> float:
    """WER via Whisper large-v3-turbo. Loads audio with soundfile to bypass FFmpeg."""
    try:
...
def compute_sim(wav_path: str, ref_wav_path: str, device: str) -> float:
    ...
    try:
        ...
        e1, e2 = embed(wav_path), embed(ref_wav_path)
...
def run_experiment(args):
    device = args.device
    out_dir = PROJECT_ROOT / "results" / "emotion_transfer"
    ...
    for row in rows:
        ...
        print(f"  {w_str}")
        wer_val = compute_wer(row["output_wav"], gen_text, device)
        sim_val = compute_sim(row["output_wav"], identity_wav_for_sim, device)
